...
const cleanPhotos = {
  async deleteAnimalsFiles() {
    ...
    pool.query(
      "SELECT photo1, photo2, photo3, photo4 FROM animal",
      (err, res) => {
        if (err) throw err;
        ...
        for (const animal of res.rows) {
          const animalPhotos = Object.values(animal).filter(
            (value) => value !== null
          );
        ...
        const missingFiles = imageFiles.filter(
          (filename) => !possibleFiles.includes(filename)
        ); // Créer un tableau des fichiers présents dans le dossier images
        if (missingFiles.length > 0) {
          // Si des fichiers sont manquants dans la bdd (donc non répertoriés)
          missingFiles.forEach((filename) => {
            // Parcours chaque nom de fichier et le supprime
